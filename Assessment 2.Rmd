...
Main_dishes <- data %>% 
  group_by(main_dish) %>% 
  count(main_dish,main_prep, family_income, us_region)

#Data vis 1
Main_dishes %>% filter(!is.na(main_dish)&!is.na(main_prep)&!is.na(us_region)) %>% 
  ggplot(aes(us_region,n, fill=main_dish))+
  geom_col()+
  coord_flip()+
  theme_light()+
  labs(title = "Main dish served at Thanksgiving by US region", x = "US Region", y = "Count of main dish")
  
#data vis 2
Main_dishes%>% filter(!is.na(main_dish)&!is.na(main_prep)&!is.na(us_region)) %>% 
  ggplot(aes(main_dish, main_prep, size = n, colour=n))+
  geom_jitter()+
  coord_flip()+
  theme_light()+
  labs(title = "Preperation type of main dishes", x = "Main dish type", y = "Preperation type")


#data vis 3
Main_dishes%>% filter(!is.na(main_dish)&!is.na(main_prep)&!is.na(us_region)) %>% 
   ggplot(aes(us_region,y=n, colour=main_prep))+
  geom_count()+
  coord_flip()+
  facet_wrap(~main_prep)+
  theme_light()+
  labs(title = "Preperation type of main dishes by US regions", x = "US Region", y = "Count of prep type")
